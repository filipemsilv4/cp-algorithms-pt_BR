...
### Finding the totient from $L$ to $R$ using the [segmented sieve](sieve-of-eratosthenes.md#crivo-segmentado) { data-toc-label="Finding the totient from L to R using the segmented sieve" }

If we need the totient of all numbers between $L$ and $R$, we can use the [segmented sieve](sieve-of-eratosthenes.md#crivo-segmentado) approach.
